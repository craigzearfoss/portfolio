@php
    ...
    if (!empty($owner) && !empty($admin) && $admin->root) {
        ...
        $breadcrumbs[] = [ 'name' => $award->name, 'href' => route('admin.portfolio.award.show', [$award, 'owner_id'=>$owner->id]) ];
    } else {
        $breadcrumbs[] = [ 'name' => 'Portfolio',  'href' => route('admin.portfolio.index') ];
        $breadcrumbs[] = [ 'name' => 'Awards',     'href' => route('admin.portfolio.award.index') ];
        $breadcrumbs[] = [ 'name' => $award->name, 'href' => route('admin.portfolio.award.show', $award) ];
    }
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.portfolio.award.update', array_merge([$award], request()->all())) }}"
            ...
            @include('admin.components.form-visibility-horizontal', [
                'public'      => old('is_public')   ?? $award->public,
                'readonly'    => old('is_readonly') ?? $award->is_readonly,
                'root'        => old('is_root')     ?? $award->root,
                'disabled'    => old('is_disabled') ?? $award->is_disabled,
                'demo'        => old('is_demo')     ?? $award->is_demo,
