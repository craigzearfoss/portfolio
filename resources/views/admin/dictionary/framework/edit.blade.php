...
@section('content')
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.dictionary.framework.update', array_merge([$framework], request()->all())) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            @include('admin.components.form-image-horizontal', [
                'src'     => old('image') ?? $framework->image,
                'credit'  => old('image_credit') ?? $framework->image_credit,
                'source'  => old('image_source') ?? $framework->image_source,
                'message' => $message ?? '',
            ])

            @include('admin.components.form-image-horizontal', [
                'name'      => 'thumbnail',
                'src'       => old('thumbnail') ?? $framework->thumbnail,
                'credit'    => false,
                'source'    => false,
                'maxlength' => 500,
                'message'   => $message ?? '',
            ...
            @include('admin.components.form-visibility-horizontal', [
                'public'      => old('is_public')   ?? $framework->is_public,
                'readonly'    => old('is_readonly') ?? $framework->is_readonly,
                'root'        => old('is_root')     ?? $framework->root,
                'disabled'    => old('is_disabled') ?? $framework->is_disabled,
                'demo'        => old('is_demo')     ?? $framework->is_demo,
