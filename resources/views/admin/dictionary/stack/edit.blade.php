...
@section('content')
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.dictionary.stack.update', array_merge([$stack], request()->all())) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            @include('admin.components.form-image-horizontal', [
                'src'     => old('image') ?? $stack->image,
                'credit'  => old('image_credit') ?? $stack->image_credit,
                'source'  => old('image_source') ?? $stack->image_source,
                'message' => $message ?? '',
            ])

            @include('admin.components.form-image-horizontal', [
                'name'      => 'thumbnail',
                'src'       => old('thumbnail') ?? $stack->thumbnail,
                'credit'    => false,
                'source'    => false,
                'maxlength' => 500,
                'message'   => $message ?? '',
            ...
            @include('admin.components.form-visibility-horizontal', [
                'public'      => old('is_public')   ?? $stack->is_public,
                'readonly'    => old('is_readonly') ?? $stack->is_readonly,
                'root'        => old('is_root')     ?? $stack->root,
                'disabled'    => old('is_disabled') ?? $stack->is_disabled,
                'demo'        => old('is_demo')     ?? $stack->is_demo,
