...
@section('content')
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.dictionary.operating-system.update', array_merge([$operatingSystem], request()->all())) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <?php /* note that you CANNOT change the owner of a dictionary operating system */ ?>
            @include('admin.components.form-hidden', [
                'name'  => 'owner_id',
                'value' => $operatingSystem->owner_id
            ])
            ...
            @include('admin.components.form-visibility-horizontal', [
                'is_public'   => old('is_public')   ?? $operatingSystem->is_public,
                'is_readonly' => old('is_readonly') ?? $operatingSystem->is_readonly,
                'is_root'     => old('is_root')     ?? $operatingSystem->root,
                'is_disabled' => old('is_disabled') ?? $operatingSystem->is_disabled,
                'is_demo'     => old('is_demo')     ?? $operatingSystem->is_demo,
