...
@section('content')
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.dictionary.category.update', array_merge([$category], request()->all())) }}" method="POST">
            ...
            @include('admin.components.form-image-horizontal', [
                'name'      => 'thumbnail',
                'src'       => old('thumbnail') ?? $category->thumbnail,
                'credit'    => false,
                'source'    => false,
                'maxlength' => 500,
                'message'   => $message ?? '',
            ...
            @include('admin.components.form-visibility-horizontal', [
                'public'      => old('is_public')   ?? $category->is_public,
                'readonly'    => old('is_readonly') ?? $category->is_readonly,
                'root'        => old('is_root')     ?? $category->root,
                'disabled'    => old('is_disabled') ?? $category->is_disabled,
                'demo'        => old('is_demo')     ?? $category->is_demo,
