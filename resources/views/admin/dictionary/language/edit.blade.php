...
@section('content')
    ...
    <div class="edit-container card form-container p-4">
        ...
        <form action="{{ route('admin.dictionary.language.update', array_merge([$language], request()->all())) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <?php /* note that you CANNOT change the owner of a dictionary language */ ?>
            @include('admin.components.form-hidden', [
                'name'  => 'owner_id',
                'value' => $language->owner_id
            ])
            ...
            @include('admin.components.form-image-horizontal', [
                'name'      => 'thumbnail',
                'src'       => old('thumbnail') ?? $language->thumbnail,
                'credit'    => false,
                'source'    => false,
                'maxlength' => 500,
                'message'   => $message ?? '',
            ...
            @include('admin.components.form-visibility-horizontal', [
                'is_public'   => old('is_public')   ?? $language->is_public,
                'is_readonly' => old('is_readonly') ?? $language->is_readonly,
                'is_root'     => old('is_root')     ?? $language->root,
                'is_disabled' => old('is_disabled') ?? $language->is_disabled,
                'is_demo'     => old('is_demo')     ?? $language->is_demo,
