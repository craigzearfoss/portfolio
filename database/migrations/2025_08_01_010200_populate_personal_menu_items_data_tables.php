...
return new class extends Migration
{
    ...
    /**
     * The id of the admin who owns the personal database resources.
    ...
    public function up(): void
    {
        ...
        if (!$database = Database::where('database', $dbName)->first()) {
            ...
            /** -----------------------------------------------------
             * Add menu items for personal database.
             ** ----------------------------------------------------- */
            $data = [
                [
                    'parent_id'   => null,
            ...
            /** -----------------------------------------------------
             * Add menu items for personal resources.
             ** ----------------------------------------------------- */
            $data = [];
            ...
            foreach (MenuItem::where('database_id', $database->id)->where('sequence', $database->sequence)->get() as $dbMenuItem) {
                ...
                foreach (Resource::where('database_id', $database->id)->get() as $resource) {
                    ...
                    $data[] = [
                        ...
                        'icon'        => $resource->icon,
                        'route'       => $routeRoot . 'personal.' . $resource->name . '.index',
