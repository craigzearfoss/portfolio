...
class AdminResourceController extends BaseAdminController
{
    ...
    public function index(Request $request): View
    {
        ...
        // by default, root admins display all admin resources
        $owner = ($this->owner && ($this->owner['id'] !== $this->admin['id'])) ? $this->owner : null;

        $adminResources = new AdminResource()->searchQuery(request()->except('id'), $owner)
            ->orderBy('owner_id')
            ->orderBy('sequence')
    ...
    {
        createGate(AdminResource::class, $this->admin);
