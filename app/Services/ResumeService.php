...
class ResumeService {
    ...
    protected function loadData(Admin $admin): void
    {
        ...
        $this->awards = new Award()->newQuery()->where('owner_id', '=', $this->admin['id'])
            ->where('is_public', '=', 1)
            ->where('is_disabled', '=', 0)
            ->orderBy('award_year')
            ->get();

        $this->certificates = new Certificate()->newQuery()->where('owner_id', '=', $this->admin['id'])
            ->where('is_public', '=', 1)
            ->where('is_disabled', '=', 0)
            ->orderBy('received', 'desc')
            ->get();

        $this->educations = new Education()->newQuery()->where('owner_id', '=', $this->admin['id'])
            ->where('is_public', '=', 1)
            ->where('is_disabled', '=', 0)
            ->orderBy('graduation_date', 'desc')
            ->get();

        $this->jobs = new Job()->newQuery()->where('owner_id', '=', $this->admin['id'])
            ->orderBy('start_date', 'desc')
            ->get();

        $this->skills = new Skill()->newQuery()->where('owner_id', '=', $this->admin['id'])
            ->where('is_public', '=', 1)
            ->where('is_disabled', '=', 0)
            ->orderBy('sequence')
            ->get();
