...
class InitPortfolio extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-alexrieger-portfolio {--silent}';
     ...
     * @var string
     */
    protected $description = 'This will populate the portfolio database with initial data for user alexrieger';
    ...
    {
        // get the admin
        if (!$admin = Admin::where('username', 'alexrieger')->first()) {
            echo PHP_EOL . 'Admin `alexrieger` not found.' . PHP_EOL . PHP_EOL;
            die;
        }
        ...
        if (!$this->teamId = $admin->admin_team_id) {
            echo PHP_EOL . 'Admin `alexrieger` is not on any admin teams.' . PHP_EOL;
        ...
        if (!$this->groupId = AdminAdminGroup::where('admin_id', $this->adminId)->first()->admin_group_id ?? null) {
            echo PHP_EOL . 'Admin `alexrieger` does not belong to any admin groups.' . PHP_EOL;
            echo 'Please fix before running this script.' . PHP_EOL . PHP_EOL;
            die;
        }
