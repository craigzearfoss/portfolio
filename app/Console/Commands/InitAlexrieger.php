...
class InitAlexrieger extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-alexreiger {--silent}';
    ...
    public function handle()
    {
        $commandSubdirectory = base_path() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Console'
            . DIRECTORY_SEPARATOR . 'Commands' . DIRECTORY_SEPARATOR . 'alexreiger' . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($commandSubdirectory . 'initSystem.php')) {
            echo PHP_EOL .'Importing System data for alexreiger ...' . PHP_EOL;
            Artisan::call('app:init-alexreiger-system --silent');
        }

        if (file_exists($commandSubdirectory . 'initPortfolio.php')) {
            echo PHP_EOL . 'Importing Portfolio data for alexreiger ...' . PHP_EOL;
            Artisan::call('app:init-alexreiger-portfolio --silent');
        }

        if (file_exists($commandSubdirectory . 'initCareer.php')) {
            echo PHP_EOL . 'Importing Career data for alexreiger  ...' . PHP_EOL;
            Artisan::call('app:init-alexreiger-career --silent');
        }

        if (file_exists($commandSubdirectory . 'initPersonal.php')) {
            echo PHP_EOL . 'Importing Personal data for alexreiger  ...' . PHP_EOL;
            Artisan::call('app:init-alexreiger-personal --silent');
        }
    }
}
