...
class InitSammalone extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-sammalone {--silent}';
    ...
    public function handle()
    {
        $commandSubdirectory = base_path() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Console'
            . DIRECTORY_SEPARATOR . 'Commands' . DIRECTORY_SEPARATOR . 'sammalone' . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($commandSubdirectory . 'initSystem.php')) {
            echo PHP_EOL .'Importing System data for sammalone ...' . PHP_EOL;
            Artisan::call('app:init-sammalone-system --silent');
        }

        if (file_exists($commandSubdirectory . 'initPortfolio.php')) {
            echo PHP_EOL . 'Importing Portfolio data for sammalone ...' . PHP_EOL;
            Artisan::call('app:init-sammalone-portfolio --silent');
        }

        if (file_exists($commandSubdirectory . 'initCareer.php')) {
            echo PHP_EOL . 'Importing Career data for sammalone  ...' . PHP_EOL;
            Artisan::call('app:init-sammalone-career --silent');
        }

        if (file_exists($commandSubdirectory . 'initPersonal.php')) {
            echo PHP_EOL . 'Importing Personal data for sammalone  ...' . PHP_EOL;
            Artisan::call('app:init-sammalone-personal --silent');
        }
    }
}
