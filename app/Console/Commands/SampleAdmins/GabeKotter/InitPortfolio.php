...
class InitPortfolio extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-gabe-kotter-portfolio {--silent}';
     ...
     * @var string
     */
    protected $description = 'This will populate the portfolio database with initial data for admin gabe-kotter.';
    ...
    {
        // get the admin
        if (!$admin = Admin::where('username', 'gabe-kotter')->first()) {
            echo PHP_EOL . 'Admin `gabe-kotter` not found.' . PHP_EOL . PHP_EOL;
            die;
        }
        ...
        if (!$this->teamId = $admin->admin_team_id) {
            echo PHP_EOL . 'Admin `gabe-kotter` is not on any admin teams.' . PHP_EOL;
        ...
        if (!$this->groupId = AdminAdminGroup::where('admin_id', $this->adminId)->first()->admin_group_id ?? null) {
            echo PHP_EOL . 'Admin `gabe-kotter` does not belong to any admin groups.' . PHP_EOL;
            echo 'Please fix before running this script.' . PHP_EOL . PHP_EOL;
            die;
        }
