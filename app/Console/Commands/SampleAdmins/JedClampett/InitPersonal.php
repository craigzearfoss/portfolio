...
class InitPersonal extends Command
{
     ...
     * @var string
     */
    protected $description = 'This will populate the personal database with initial data for admin jed-clampett.';
    ...
    {
        // get the admin
        if (!$admin = Admin::where('username', 'jed-clampett')->first()) {
            echo PHP_EOL . 'Admin `jed-clampett` not found.' . PHP_EOL . PHP_EOL;
            die;
        }
        ...
        if (!$this->teamId = $admin->admin_team_id) {
            echo PHP_EOL . 'Admin `jed-clampett` is not on any admin teams.' . PHP_EOL;
        ...
        if (!$this->groupId = AdminAdminGroup::where('admin_id', $this->adminId)->first()->admin_group_id ?? null) {
            echo PHP_EOL . 'Admin `jed-clampett` does not belong to any admin groups.' . PHP_EOL;
            echo 'Please fix before running this script.' . PHP_EOL . PHP_EOL;
            die;
        }
