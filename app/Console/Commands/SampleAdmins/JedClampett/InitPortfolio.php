...
class InitPortfolio extends Command
{
    ...
    {
        // get the admin
        if (!$admin = Admin::where('username', 'jed-clapmett')->first()) {
            echo PHP_EOL . 'Admin `jed-clapmett` not found.' . PHP_EOL . PHP_EOL;
            die;
        }
        ...
        if (!$this->teamId = $admin->admin_team_id) {
            echo PHP_EOL . 'Admin `jed-clapmett` is not on any admin teams.' . PHP_EOL;
        ...
        if (!$this->groupId = AdminAdminGroup::where('admin_id', $this->adminId)->first()->admin_group_id ?? null) {
            echo PHP_EOL . 'Admin `jed-clapmett` does not belong to any admin groups.' . PHP_EOL;
            echo 'Please fix before running this script.' . PHP_EOL . PHP_EOL;
            die;
        }
