...
class InitJrewing extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-jrewing {--silent}';
    ...
    public function handle()
    {
        $commandSubdirectory = base_path() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Console'
            . DIRECTORY_SEPARATOR . 'Commands' . DIRECTORY_SEPARATOR . 'jrewing' . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($commandSubdirectory . 'initSystem.php')) {
            echo PHP_EOL .'Importing System data for jrewing ...' . PHP_EOL;
            Artisan::call('app:init-jrewing-system --silent');
        }

        if (file_exists($commandSubdirectory . 'initPortfolio.php')) {
            echo PHP_EOL . 'Importing Portfolio data for jrewing ...' . PHP_EOL;
            Artisan::call('app:init-jrewing-portfolio --silent');
        }

        if (file_exists($commandSubdirectory . 'initCareer.php')) {
            echo PHP_EOL . 'Importing Career data for jrewing  ...' . PHP_EOL;
            Artisan::call('app:init-jrewing-career --silent');
        }

        if (file_exists($commandSubdirectory . 'initPersonal.php')) {
            echo PHP_EOL . 'Importing Personal data for petergibbons  ...' . PHP_EOL;
            Artisan::call('app:init-jrewing-personal --silent');
        }
    }
}
