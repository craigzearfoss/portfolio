...
class InitJedclampett extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-jedclampett {--silent}';
    ...
    public function handle()
    {
        $commandSubdirectory = base_path() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Console'
            . DIRECTORY_SEPARATOR . 'Commands' . DIRECTORY_SEPARATOR . 'jedclampett' . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($commandSubdirectory . 'initSystem.php')) {
            echo PHP_EOL .'Importing System data for jedclampett ...' . PHP_EOL;
            Artisan::call('app:init-jedclampett-system --silent');
        }

        if (file_exists($commandSubdirectory . 'initPortfolio.php')) {
            echo PHP_EOL . 'Importing Portfolio data for jedclampett ...' . PHP_EOL;
            Artisan::call('app:init-jedclampett-portfolio --silent');
        }

        if (file_exists($commandSubdirectory . 'initCareer.php')) {
            echo PHP_EOL . 'Importing Career data for jedclampett  ...' . PHP_EOL;
            Artisan::call('app:init-jedclampett-career --silent');
        }

        if (file_exists($commandSubdirectory . 'initPersonal.php')) {
            echo PHP_EOL . 'Importing Personal data for jedclampett  ...' . PHP_EOL;
            Artisan::call('app:init-jedclampett-personal --silent');
        }
    }
}
