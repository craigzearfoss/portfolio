...
class InitTonynelson extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:init-tonynelson {--silent}';
    ...
    public function handle()
    {
        $commandSubdirectory = base_path() . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'Console'
            . DIRECTORY_SEPARATOR . 'Commands' . DIRECTORY_SEPARATOR . 'tonynelson' . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($commandSubdirectory . 'initSystem.php')) {
            echo PHP_EOL .'Importing System data for tonynelson ...' . PHP_EOL;
            Artisan::call('app:init-tonynelson-system --silent');
        }

        if (file_exists($commandSubdirectory . 'initPortfolio.php')) {
            echo PHP_EOL . 'Importing Portfolio data for tonynelson ...' . PHP_EOL;
            Artisan::call('app:init-tonynelson-portfolio --silent');
        }

        if (file_exists($commandSubdirectory . 'initCareer.php')) {
            echo PHP_EOL . 'Importing Career data for tonynelson  ...' . PHP_EOL;
            Artisan::call('app:init-tonynelson-career --silent');
        }

        if (file_exists($commandSubdirectory . 'initPersonal.php')) {
            echo PHP_EOL . 'Importing Personal data for tonynelson  ...' . PHP_EOL;
            Artisan::call('app:init-tonynelson-personal --silent');
        }
    }
}
